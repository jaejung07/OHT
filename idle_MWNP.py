...
class Map:
    ...
    def draw_map(self):
        for node in self.nodes.values():
            create_magged_oval(node.coordinate[0], node.coordinate[1], 1, "black")
        
        for station in self.stations.values():
            create_magged_oval(station.coordinate[0], station.coordinate[1], 1, "blue")
...
class Vehicle:
    ...
    def __init__(self, x, y, location):
        self.id = create_magged_oval(x, y, 4, "red")
        self.speed = 0
        self.coordinate = [x, y]
    ...
    def next_step(self, displacement, map, policy):
        ...
        magged_moveto(self.id, self.coordinate[0], self.coordinate[1], 4)
            ...
            if (type(vehicle.location) == Node and (vehicle.location == self.location.end and self.location.distance(self.coordinate) < self.safe_distance) or \
                ...
                if vehicle.state == "idle":
                    ...
                    if type(vehicle.location) == Node:
                        ...
                        vehicle.route = [vehicle.location.end]
                    vehicle.start_frame = frame
                return "crash"
