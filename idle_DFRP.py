...
class Sim:
    ...
    def __init__(self, m, p):
        ...
        with open("newDM2.csv", "w") as file:
...
class Policy:
    def __init__(self, map):
        ...
        for station in map.stations.keys():
            ...
            train = self.df0[station]['PULocationID'][:1]
            
            self.model[station] = pm.auto_arima(train, d=1, seasonal=False, trace=True)
...
#Deterministic하게
    ...
    def demand_forecaster(self, map):
        for station in map.stations.keys():
            fc, conf = forcast_one_step(self.model[station])
            self.demand_forecast[map.stations[station]] = fc
            try:
                self.model[station].update(self.df0[station]['PULocationID'][frame//WINDOW])
            except:
                pass
