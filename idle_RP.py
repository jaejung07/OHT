...
class Sim:
    ...
    def __init__(self, m, p):
        ...
        with open("newDDDD.csv", "w") as file:
...
class Vehicle:
    ...
    def passed(self, map, p, road, frame):
        ...
        if self.start_frame != 0:
            road.weight = (1 - self.weight_rambda) * road.weight + self.weight_rambda * (frame - self.start_frame)
        #경로 재탐색
...
#Deterministic하게
    ...
    def demand_forecaster(self, map):
        for station in map.stations.keys():
            try:
                self.demand_forecast[map.stations[station]] = self.df0[station]['PULocationID'][frame//WINDOW]
            except:
                pass

    def demand_forecaster2(self, map):
        pass
    ...
    def idleer_DM(self, map):
        ...
        for vehicle in vehicles:
            ...
            if available:
                current_sol.append(available[0])
                max_E = 0
                for node in available:
                    adjacent_nodes = list(map.roadss[node].keys())
                    E = [lambdaA * self.demand_forecast[adjacent_node] for adjacent_node in adjacent_nodes]
                    E.append(self.demand_forecast[node] * 1)
                    if max(E) > max_E:
                        max_E = max(E)
                        current_sol[-1] = node
