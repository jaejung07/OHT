...
class Sim:
    ...
    def __init__(self, m, p):
        ...
        deliveries.append(('gen_time', 'assign_time', 'PU', 'DO', 'vehicle', 'pickup', 'done'))
        self.xlcount = 0
    ...
    def vehicle_generator(self, num):
        ...
        for i in range(num):
            ...
            for vehicle in vehicles:
                occupied_nodes.add(vehicle.location)
            ...
            #vehicles[-1].random_move(self.m)
            vehicles[-1].route = self.p.idleer_C(vehicles[-1].location, self.m)
            vehicles[-1].location = self.m.roadss[vehicles[-1].location][vehicles[-1].route[0]]
    ...
    def delivery_generator(self):
        ...
        #무작위 생성
            ...
            gen_time = self.df.iloc[self.xlcount, 1]
            NewDel = Delivery(chosen_start, chosen_end, gen_time, 1, self.p, self.m)
            
            if NewDel.assigned == "NAV":
                break
            else:
                self.xlcount += 1
    ...
    def weight_initiator(self):
        ...
        for roads in self.m.roadss.values():
            for road in roads.values():
                if type(road) == Curve:
                    road.weight = (road.distance(road.start.coordinate) / vehicle_one.curved_speed)

                elif type(road) == Line:
                    road.weight = (road.distance(road.start.coordinate) / vehicle_one.linear_speed)
...
class Vehicle:
    ...
    def accel_or_decel(self, map):
        look_ahead = self.look_ahead(map)
    ...
    def done(self, map, p):
        deliveries.append((self.gen_time, self.assign_time, self.departure.name, self.arrival.name, self.id, self.pickup_time, frame))
        self.gen_time = None
    ...
    def look_ahead(self, map):
        ...
        for vehicle in self_deleted_vehicles:
            if vehicle.location == self.location and 0 < (self.location.distance(self.coordinate) - self.location.distance(vehicle.coordinate)) < self.safe_distance:
                return "crash"
    ...
    def assigner(self, delivery, map):
        ...
        while queue and len(queue) > 0:
            ...
            for roads in list(map.roadss.values()):
                ...
                if road:
                    for vehicle in vehicles:
                        if vehicle.location == road and vehicle.state == "idle":
        ...
        if not available_vehicles:
            ...
            chosen_vehicle.gen_time = delivery.gen_time
...
#Deterministic하게
    ...
    #디맨드만 딥러닝으로
    def idleer_DM(self, vehicle, map):
        #Demand Forecasting
        pass
...
class Delivery:
    def __init__(self, departure, arrival, time, weight, policy, map):
        self.departure = departure
        self.arrival = arrival
        self.weight = weight
        self.gen_time = time
        res = policy.assigner(self, map)
